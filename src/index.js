...
document.addEventListener('DOMContentLoaded', () => {
    ...
    const revealPick = (index, team, delay, runID) => {
        setTimeout(() => {
    ...
    const revealResultID = (resultID, delay, runID) => {
        setTimeout(() => {
            if (runID === currentRunID) {
    ...
    startButton.addEventListener('click', async () => {
        ...
        if (lotteryOrder.length > 1) {
            totalDelay = revealPick(1, lotteryOrder[1], totalDelay + ONE_SECOND * 5, runID);
        }

        // Reveal pick 1
        if (lotteryOrder.length > 0) {
            totalDelay = revealPick(0, lotteryOrder[0], totalDelay + ONE_SECOND, runID);
        }

        // Reveal resultID
        revealResultID(resultID, totalDelay + ONE_SECOND, runID);
    });
});